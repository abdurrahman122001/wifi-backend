...
const uploadDir = path.join(__dirname, '../uploads/silder');
const logoBase = path.join(uploadDir, 'silder');
...
router.get('/', (req, res) => {
  const files = fs.readdirSync(uploadDir).filter(f => f.startsWith('silder.'));
